  ...
  useEffect(() => {
    const fetchDriverData = async () => {
        try {
            ...
            const response = await axios.get(`http://192.168.1.82:3001/getDriver/${driverId}`);
            if (response.data) {
...
const currentRecords = finalSortedRecords.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  const [signature, setSignature] = useState(null);
    ...
    useEffect(() => {
      const fetchSignature = async () => {
        try {
          const response = await fetch(`http://192.168.1.82:3001/getSignature/${selectedRecord._id}`);
  ...
  return (
    <div>
      ...
      {/* Modal */}
      {isModalOpen && (
          <div className="modal-overlay1 active">
            <div className="modal-content1">
              ...
              <div style={{ fontFamily: "Arial, sans-serif", width: "100%" }}>
                    <div className="receipt-row1">
                      ...
                      <p><strong>{new Date(selectedRecord.timeOfApprehension).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong></p>
                    </div>
                    <p style={{textAlign: "center", fontFamily:"courier", marginBottom:'-5px', marginTop:"-5px"}}>====================================================</p>
                    <div className="receipt-row">
                      <p>Driver's Name:</p>
                      <p><strong>{selectedRecord.nameOfDriver}</strong></p>
                    </div>
                    <div className="receipt-row">
                      <p>License Number:</p>
                      <p><strong>{selectedRecord.licenseNumber}</strong></p>
                    </div>
                    <div className="receipt-row">
                      <p>Plate Number:</p>
                    ...
                    <p style={{ textAlign: "center", fontFamily:"courier", marginBottom:'-5px', marginTop:"-5px" }}>====================================================<br/> BRING THIS RECEIPT DURING PAYMENT <br/>PAY WITHIN 7 DAYS</p>
                    </div>
