...
import { faSort, faArrowUpZA, faMapLocation, faCertificate, faXmark, faDownload } from '@fortawesome/free-solid-svg-icons';
      ...
      if (driverId) {
        ...
        if (officerResponse.ok) {
          ...
          if (officerData.role === 'Admin') {
            ...
            setAgency(officerData.agency); // ✅ Save the agency here
...
const handleFineUpdate = async (recordId) => {
  ...
  const handleClearanceSubmit = async (e) => {
    ...
    const recordData = {
      ...
      name: ictpmoRecord.nameOfDriver || "N/A",
      address: ictpmoRecord.address || "N/A",
      vehicle: vehicle,
      brand: brand,
      plateNumber: plate,
      chassisNumber: cha,
      ...
      oRNo: ictpmoRecord.oRNo || "N/A",
      datepaid: ictpmoRecord.datepaid || new Date().toISOString(),
      fineAmount: ictpmoRecord.fineAmount || 0,
      submissionDate: new Date().toISOString(),
    };
  
    try {
      console.log("Submitting clearance record:", recordData); // Debug log
  
      const response = await fetch("http://192.168.1.4:3001/addClearance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        ...
        console.log("Clearance record saved successfully!");
      } else {
        throw new Error("Failed to save record to the database");
      }
    } catch (error) {
      console.error("Error saving clearance record:", error);
      alert("Failed to save the clearance record. Please try again.");
    }
  };
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      setTicketNumber3((prev) => {
        const newNumber = (parseInt(prev) + 1).toString().padStart(5, "0");
        localStorage.setItem("lastTicketNumber3", newNumber);
        return newNumber;
      });

      await handleClearanceSubmit({ preventDefault: () => {} });
...
const handleEdit = (records) => {
  ...
  setFormState({
    ...
    oRNo: records.oRNo || "",
    datepaid: records.datepaid ? records.datepaid.split("T")[0] : "",
  });
...
const handleDelete = async (recordId) => {
  ...
  try {
    const response = await fetch(`http://192.168.1.32:3001/deleteRecord/${recordId}`, {
      method: 'DELETE',
    });
...
const handleSubmit = async (e) => {
    ...
    try {
      if (selectedData) {
        ...
        await updateDoc(recordDocRef, { ...formState, fineAmount });
        alert("Data updated successfully!");
      } else {
        await addDoc(collection(db, "records"), { ...formState, fineAmount });
  ...
  // Update fine amount when violation type or record source changes
  useEffect(() => {
...
const finalSortedRecords = [...filteredData]
  ...
  .filter(record => record.fineStatus == "Paid")
               ...
               <th style={{ width: "60px" }} onClick={() => handleSort('ticketNumber')}>Ticket No.</th>
                ...
                <th style={{ width: "80px" }} onClick={() => handleSort('nameOfDriver')}>Driver' Name</th>
                   ...
                   <th style={{ width: "50px" }} >Vehicle Type</th>
                    ...
                    <th style={{ width: "40px" }} > O.R. No. </th>
                    <th style={{ width: "40px" }}> Date Paid </th>
              ...
              {loading ? (
                  ...
                  currentRecords.map((records) => (
                    <tr key={records?._id}>
                      ...
                      <td>{records?.oRNo} </td>
                      <td>{records?.datepaid ? records.datepaid.split("T")[0] : "N/A"}</td>
                    <td>
                      ...
                      {agency === "ICTPMO" && (
                        <button className="clearance-button" onClick={() => handleClearance(records)}>
...
<Map onCsvUpload={handleCsvData} />
        ...
        <Modal isOpen={isModalOpen} onRequestClose={closeModal} className="modal-contentovr" overlayClassName="overlayovr1">
              ...
              <select 
               ...
               <div className="form-groupovr">
                ...
                <label htmlFor="orno">OR No.  <span style={{ color: 'red' }}>*</span></label>
                <input 
                  id="orno" 
                  type="text" 
                ...
                <label htmlFor="datepaid">Date Paid  <span style={{ color: 'red' }}>*</span></label>
                <input 
                  id="datepaid" 
                  type="text" 
            ...
            {/* Clearance content */}
            <Modal isOpen={showModal} onRequestClose={closeModal} className="modal-clearance" overlayClassName="overlay">
              <div>
                <div className="clearancemodalheader">
                  <h2>Traffic Clearances</h2>
                  <FontAwesomeIcon onClick={closeModal} icon={faXmark} size="s" style={{color: "#f44336", border:"2px solid #f44336", borderRadius:"80px", padding:"5px"}} />
                </div>
                  <h4>Select the type of clearance</h4>
              </div>
              <div className="tabs">
                <button onClick={() => setActiveTab("tab1")} className={activeTab === "tab1" ? "active-tab" : ""}>Lost Driver's License</button>
                <button onClick={() => setActiveTab("tab2")} className={activeTab === "tab2" ? "active-tab" : ""}>For Vehicle Registration</button>
                <button onClick={() => setActiveTab("tab3")} className={activeTab === "tab3" ? "active-tab" : ""}>Lost Certificate of Registration/Official Receipt</button>
              </div>
              ...
              <div className="tab-content">
                ...
                <div>
                  ...
                  <div className="clearcontent"> 
                    <h1 className="headtc">TRAFFIC CLEARANCE</h1>
                    <h3>TO WHOM IT MAY CONCERN;</h3>
                    <p>
                      &emsp;&emsp;&emsp;THIS IS TO CERTIFY that as per record available in this office shows that
                      {" "}<strong>{ictpmoRecord.nameOfDriver|| "N/A"}</strong> {" "}
                      , of legal age, Filipino, single/married/widow/er a resident of
                      {" "}<strong>{ictpmoRecord.address|| "N/A"}</strong> {" "}
                  ...
                  <div className="signatories21">
                    <p>Data Encoder	   </p>
                    <p>Admin In-charge ICTPMO </p>
                  </div>
                  <div className="seal-clear">
                    <p>O.R. No. &emsp;&emsp;: 
                    {" "}{ictpmoRecord.oRNo|| "N/A"}
                    </p>
                    <p>Amount  &emsp;&emsp;&emsp;: 
                    {" ₱ "}{ictpmoRecord.fineAmount|| "N/A"}
                    </p>
                    <p>Date    &emsp;&emsp;&emsp;&emsp;: 
                    {" "}{ictpmoRecord.datepaid 
                      ? new Date(ictpmoRecord.datepaid).toLocaleDateString() 
                      : "N/A"}
                    </p>
                  </div><br/>
                </div>
              ...
              <h1 className="headtc">TRAFFIC CLEARANCE</h1>
                  <h3>TO WHOM IT MAY CONCERN;</h3>
                  <p>
                    &emsp;&emsp;&emsp;This is to certify that based on records available in this office shows that the CERTIFICATE OF REGISTRATION/OFFCIAL RECEIPT issued under the name of
                    {" "}<strong>{ictpmoRecord.nameOfDriver|| "N/A"}</strong> {" "}
                    , a 
                    {" "}<input
                      type="text"
   ...
   <h1 className="headtc">TRAFFIC CLEARANCE</h1>
                  ...
                  <p>
                    ...
                    />                  
                    to support his/her application for the replacement/renewal of his/her Official Receipt/Certificate of Registration at 
                    <input
                        type="text"
                        className="editable-input"
                        placeholder="Address"
