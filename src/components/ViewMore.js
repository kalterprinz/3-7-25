...
const ViewMore = () => {
  ...
  const fetchDriver = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.1.82:3001/getdriverfromrecords/${encodeURIComponent(record.email)}`);
  ...
  useEffect(() => {
    const fetchSignature = async () => {
      try {
        const response = await fetch(`http://192.168.1.82:3001/getSignature/${record._id}`);
