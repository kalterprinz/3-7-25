...
useEffect(() => {
  const fetchComplianceData = async () => {
    try {
      ...
      const driverRes = await fetch(`http://192.168.43.245:3001/getDriver/${driverId}`);
      const driverData = await driverRes.json();
      const driverEmail = driverData?.email;

      if (!driverEmail) return;

      const recordsRes = await fetch("http://192.168.43.245:3001/getRecords");
  ...
  return (
    <div className="landing-page">
        ...
        <header className="headerdriver">
          ...
          <h4 className="titled2">ILIGAN CITATION TICKET ONLINE</h4>
          <nav className="navy">
