...
const fetchDoughnutData = async () => {
  try {
    ...
    const driverResponse = await fetch(`http://192.168.1.32:3001/getDriver/${driverId}`);
    ...
    // Fetch all records
    const response = await fetch("http://192.168.1.32:3001/getRecords");
...
useEffect(() => {
  const fetchComplianceData = async () => {
    try {
      ...
      const driverRes = await fetch(`http://192.168.1.32:3001/getDriver/${driverId}`);
      const driverData = await driverRes.json();
      const driverEmail = driverData?.email;

      if (!driverEmail) return;

      const recordsRes = await fetch("http://192.168.1.32:3001/getRecords");
  ...
  return (
    <div className="landing-page">
        ...
        <div className="page-headerDF">
           <h2 style={{color:"#2b4450", fontSize:"25px", marginBottom:"0px"}}><FontAwesomeIcon icon={faIdCard} style={{ marginRight: "20px" , marginLeft: "5px" }} />Profile</h2>
                ...
                <div className="pie-chart">
                  <button className="legend-button" onClick={toggleLegend} >
                      ...
                      <p>Indicates how well the driver adheres to traffic rules based on their violation record</p>
                          ...
                          <div style={{ textAlign: "center", marginTop: "10px", display:"inline-flex"}}>
                            <strong>
                ...
                <div className="pie-chart1">
                    ...
                    <Doughnut
                      data={{
                        ...
                        datasets: [
                          {
                            data: Object.values(violationTypeData),
                            backgroundColor: [
        ...
        {/* Records Section */}
        <div className="page-headerDf">
            <h2 style={{color:"#2b4450", fontSize:"25px", borderBottomStyle:"solid", margin:"30px 90px", borderBottomColor:"#2b4450"}}><FontAwesomeIcon icon={faRectangleList} style={{ marginRight: "20px" , marginLeft: "5px" }} />Transaction</h2>
          </div>
        <section id="Records" className="Records">
            <div className="records-content">
                <div className="left">
                  <div className="records-tableD">
                      <DriverRecords/>
                  </div>
                </div>
                <div className="right">
