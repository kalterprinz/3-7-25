...
function startBackendInstance() {
  ...
  // Connect to MongoDB
  mongoose.connect('mongodb://192.168.1.82:27017/thesis')
...
app.post('/addRecord', async (req, res) => {
  ...
  console.log('Received data:', req.body);
    ...
    const { 
        ...
        licenseClassification, dueDate, agency, signature
    } = req.body;

    try{
        let processedSignature = undefined;

        if (signature && signature.data && signature.contentType) {
          // Remove data URL prefix (e.g., "data:image/png;base64,...")
          const base64Data = signature.data.replace(/^data:image\/\w+;base64,/, "");
        ...
        res.status(201).json({
            ...
            record: newRecord, // Send the saved record data as part of the response (optional)
        });
...
app.get('/getRecords', async (req, res) => {
  try {
      ...
      const records = await RecordModel.find(filter).sort({ _id: -1 }); // Sorting by _id in descending order

      if (records.signature && records.signature.data) {
        records.signature = {
          data: records.signature.data.toString("base64"),
          contentType: records.signature.contentType,
        };
      }
